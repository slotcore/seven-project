...
'*****************************************************************************************************
'* Nombre Archivo   : FRMMANTC.FRM
'* Tipo             : FORMULARIO
'* Descripcion      : FORMULARIO QUE PERMITE LAS ALTAS Y BAJAS EN LA TABLA con_tc
...
Dim RstTC As New ADODB.Recordset       ' RECORDSET PARA ALMACENAR LOS REGISTROS DE LA TABLA con_tc
Dim RsCons As New ADODB.Recordset
Dim QueHace As Integer                 ' VARIABLE QUE ESPECIFICA EN QUE ESTADO SE ENCUENTRA EL FORMULARIO
Dim SeEjecuto As Boolean               ' VARIABLE QUE CONTRALO QUE EL EVENTO ACTIVATE SE EJECUTE UNA SOLA VEZ
Dim xTitulo As String                  ' ESPECIFICA EL TITULO PARA LOS CUADRO MSGBOX
Dim CaracteresNumericos As String      ' ESPECIFICA LOS CARACTERES NUMERICOS QUE SE PODRAN USAR EN LOS CONTROLES TEXTBOX
Dim vStrSql As String                  ' ALAMCENARA LAS CADENAS SQL QUE SE VAYAN A GENERAR
Dim Oleapp As Object
Dim BAND_INTERRUMPIR As Boolean        ' CONTROLA EL AVANCE DE LOS BUCLES FOR, DO Y PROGRESBAR
...
'*****************************************************************************************************
'* Nombre           : fverif_tc_deundia
'* Tipo             : PROCEDIMIENTO
'* Descripcion      : FUNCION QUE VERIFICA SI EXISTE EL TIPO DE CAMBIO EN EL DIA ESPECIFICADO, ESTA
'*                    FUNCION DEVUELVE VERDERO EN CASO DE ENCONTRAR UN TIPO DE CAMBIO EN EL DIA
'*                    ESPECIFICADO
'* Paranetros       :
'* Devuelve         : Boolean
'*****************************************************************************************************
...
'* Nombre           : confpredeter
'* Tipo             : PROCEDIMIENTO
'* Descripcion      : CONFIGURA LOS VALORES PREDETERMINADOS PARA LOS CONTROLES TxtAnio, UpDown1
'* Paranetros       :
'* Devuelve         :
'*****************************************************************************************************
Sub confpredeter()
...
'* Nombre           : centrarFrame
'* Tipo             : PROCEDIMIENTO
'* Descripcion      : CENTRA EN EL FORMULARIO EL CONTROL FraRegistrar
'* Paranetros       :
'* Devuelve         :
'*****************************************************************************************************
Sub centrarFrame()
...
'* Nombre           : LlenarGrid
'* Tipo             : PROCEDIMIENTO
'* Descripcion      : LLENA EL CONTROL Dg1 CON DATOS DE LA TABLA con_tc
'* Paranetros       : NOMBRE          |  TIPO     |  DESCRIPCION
'*                    --------------------------------------------------------------------------------
'*                    pConFiltroSiNO  |  Boolean  |  ESPECIFIVA SI SE APLICA UN FILTRO
'* Devuelve         :
'*****************************************************************************************************
Sub LlenarGrid(pConFiltroSiNO As Boolean)
    ...
    Select Case pConFiltroSiNO
        Case True 'SI
            RST_Busq RstTC, "SELECT con_tc.id, con_tc.fecha, mae_moneda.descripcion,mae_moneda.simbolo, con_tc.impcom, con_tc.impven " _
...
'*****************************************************************************************************
'* Nombre           : LlenarTextosPaModifi
'* Tipo             : PROCEDIMIENTO
'* Descripcion      : CARGA LOS DATOS DEL REGISTRO ACTUAL
'* Paranetros       :
'* Devuelve         :
'*****************************************************************************************************
...
Private Sub Dg1_KeyUp(KeyCode As Integer, Shift As Integer)
    If KeyCode = 123 Then
        VerMovimientos1 IdMenuActivo, NulosN(RstTC("id")), xCon
    End If
End Sub
...
Private Sub Form_Activate()
    ...
    If SeEjecuto = False Then
        ...
        RST_Busq RsCons, "SELECT id, descripcion From mae_moneda Where id <>0 ORDER BY descripcion", xCon
        ...
        If RsCons.RecordCount <> 0 Then
            RsCons.MoveFirst
            DtcTMoneda_Filtro.Text = NulosC(RsCons("descripcion"))
            DtcTMoneda.Text = NulosC(RsCons("descripcion"))
        End If
        Set RsCons = Nothing
...
Private Sub Form_Load()
    ' PRIMER EVENTO EJECUTARSE CUANDO SE CARGUE EL FORMULARIO
    SeEjecuto = False
    QueHace = 2
End Sub
...
'* Nombre           : Blanquea
'* Tipo             : PROCEDIMIENTO
'* Descripcion      : INICIALIZA LOS CONTROLES TEXTBOX PARA EL INGRESO DE UN NUEVO REGISTRO
'* Paranetros       :
'* Devuelve         :
'*****************************************************************************************************
Sub Blanquea()
...
'* Nombre           : Eliminar
'* Tipo             : PROCEDIMIENTO
'* Descripcion      : ELIMINA UN REGISTRO DE LA TABLA con_tc
'* Paranetros       :
'* Devuelve         :
'*****************************************************************************************************
Sub Eliminar()
    ...
    If Rpta = vbYes Then
        ...
        xCon.Execute "DELETE * FROM var_edicion WHERE idmov = " & RstTC("id") & " AND idform = " & IdMenuActivo
...
'* Nombre           : Grabar
'* Tipo             : PROCEDIMIENTO
'* Descripcion      : GRABA UN REGISTRO EN LA TABLA con_tc, ESTA FUNCION DEVUELVE VERDADERO CUANDO
'*                    TIENE EXITO
'* Paranetros       :
'* Devuelve         : Boolean
'*****************************************************************************************************
Function Grabar() As Boolean
    ...
    If QueHace = 1 Then
        ' SI ES UN NUEVO REGISTRO, OBTENEMOS EL ULTIMO ID DE LA TABLA con_tc
    ...
    Else
        xId = RstTC("id")
        RST_Busq RstGra, "SELECT * FROM con_tc WHERE id = " & xId & "", xCon
    End If
    
    RstGra("fecha") = Trim(TxtFechaRegistro.Valor)
    RstGra("idmon") = DtcTMoneda.BoundText
    RstGra("impcom") = NulosN(TxtTC_Compra.Text)
    RstGra("impven") = NulosN(TxtTC_Venta.Text)
    RstGra.Update
    ...
    MsgBox "El tipo de cambio se grabo con éxito", vbInformation + vbOKOnly + vbDefaultButton1, xTitulo
...
'* Nombre           : Cancelar
'* Tipo             : PROCEDIMIENTO
'* Descripcion      : CANCELA EL INGRESO MODIFICACION DE UN REGISTRO
'* Paranetros       :
'* Devuelve         :
'*****************************************************************************************************
Sub Cancelar()
...
'* Tipo             : PROCEDIMIENTO
'* Descripcion      : IMPORTA A MS EXCEL LOS DATOS CARGADOS EN EL RECORDSET
'* Paranetros       :
'* Devuelve         :
'*****************************************************************************************************
...
'*****************************************************************************************************
'* Nombre           : pCrearFormato
'* Tipo             : PROCEDIMIENTO
'* Descripcion      : CREA EL FORMATO PARA IMPORTAR DATOS DESDE MS EXCEL
'* Paranetros       :
'* Devuelve         :
'*****************************************************************************************************
...
'* Nombre           : pImportar
'* Tipo             : PROCEDIMIENTO
'* Descripcion      : IMPORTA DATOS DESDE UN ARCHIVO DE MS EXCEL
'* Paranetros       :
'* Devuelve         :
'*****************************************************************************************************
...
'* Nombre           : Exportar
'* Tipo             : PROCEDIMIENTO
'* Descripcion      : EXPORTA A MS EXCEL
'* Paranetros       :
'* Devuelve         :
'*****************************************************************************************************
...
Private Sub FraRegistrar_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    OrigFX = X
    OrigFY = Y
    FraRegistrar.ZOrder 0
End Sub

Private Sub FraRegistrar_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If Button <> 0 Then
        With FraRegistrar
...
Private Sub fraImportar_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    OrigFX = X
    OrigFY = Y
    FraImportar.ZOrder 0
End Sub

Private Sub fraImportar_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If Button <> 0 Then
        With FraImportar
            .Move .Left + X - OrigFX, .Top + Y - OrigFY
        End With
