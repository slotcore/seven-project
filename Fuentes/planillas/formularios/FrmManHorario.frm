...
Private Sub pRegistroAdd(Index As Integer, Optional fSeleccionVarios As Boolean = False)
    ...
    Select Case Index
        Case 0 '--
            ...
            xCampos(0, 0) = "Personal":     xCampos(0, 1) = "nombres":   xCampos(0, 2) = "6500":     xCampos(0, 3) = "C":    xCampos(0, 4) = "S"
            xCampos(1, 0) = "Sexo":         xCampos(1, 1) = "sexo":      xCampos(1, 2) = "1000":     xCampos(1, 3) = "C":    xCampos(1, 4) = "N"
            ...
            nSQL = "SELECT pla_empleados.id, [pla_empleados].[apepat] & ' ' & [pla_empleados].[apemat] & ' ' & [pla_empleados].[nom] AS nombres, mae_sexo.abrev AS sexo " _
                + vbCr + " FROM (mae_sexo RIGHT JOIN pla_empleados ON mae_sexo.id = pla_empleados.idsex) INNER JOIN pla_periodolaboral ON pla_empleados.id = pla_periodolaboral.idemp " _
                + vbCr + " WHERE (((pla_empleados.id) Not In (select  idemp  from  mae_horarioemp where vigencia = -1))) " & nSQLId & " AND pla_periodolaboral.fchfin Is Null "
...
Private Sub Form_Activate()
    ...
    If RstFrm.RecordCount = 0 Then
        If MsgBox("No se ha registrado ningun Horario, ¿Desea agregar uno ahora?", vbQuestion + vbYesNo, xTitulo) = vbYes Then
            nuevo
        End If
    End If
End Sub
...
Function Grabar() As Boolean
    ...
    If QueHace = 1 Then
        ...
        xCod = HallaCodigoTabla("mae_horario", xCon, "id")
        RstCab.AddNew
        RstCab("id") = xCod
    Else
        xCod = RstFrm("id")
        
        RST_Busq RstCab, "SELECT * FROM mae_horario WHERE id =" & RstFrm("id") & "", xCon
        '--Eliminando los tipos de horas asignados al horario
        xCon.Execute "DELETE * FROM mae_horariohora WHERE idhor = " & RstFrm("id") & ""
        '--DESACTIVANDO LAS PERSONAS ASOCIADAS AL HORARIO
        xCon.Execute "UPDATE mae_horarioemp SET mae_horarioemp.vigencia = 0 WHERE (((mae_horarioemp.idhor)=" & xCod & "));"
    ...
    With Fg(0)
        For xFil = 1 To .Rows - 1
            nSQL = "SELECT mae_horarioemp.idemp FROM mae_horarioemp " _
                + vbCr + " WHERE (((mae_horarioemp.idhor)=" & xCod & ") AND ((mae_horarioemp.idemp)=" & NulosN(.TextMatrix(xFil, 1)) & "));"
            RST_Busq RstTmp, nSQL, xCon
            If RstTmp.RecordCount <> 0 Then
                xCon.Execute "UPDATE mae_horarioemp SET vigencia=-1 WHERE idhor=" & xCod & " AND idemp=" & NulosN(.TextMatrix(xFil, 1)) & " ;"
            Else
                RstDet.AddNew
                RstDet("idhor") = xCod
