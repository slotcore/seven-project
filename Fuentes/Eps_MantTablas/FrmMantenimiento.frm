...
Private Sub CmdBusca_Click(Index As Integer)
    ...
    Dim xForm As New eps_librerias.FormBuscar '   EPS_Buscar.Buscar
    ...
    For x = LBound(xVincula) To UBound(xVincula)
        If xCampos(Index, 1) = xVincula(x, 6) Then
            ...
            If xRs.State = 1 Then
                ...
                LblDato(Index).Caption = NulosC(xRs(xVincula(x, 7)))
            End If
            Set xForm = Nothing
            Set xRs = Nothing
        End If
...
Private Sub Db1_KeyUp(KeyCode As Integer, Shift As Integer)
    If KeyCode = 123 And TabOne1.CurrTab = 0 Then
        Dim xFunc As Object
        Set xFunc = CreateObject("SGI2_funciones.varias")
        xFunc.VerMovimientos1 xIdMenu, RstCab(xCampCla), xConeccion
        Set xFunc = Nothing
...
Private Sub Form_Activate()
    If SeEjecuto = False Then
        ...
        Dim xFunc As Object
        Set xFunc = CreateObject("SGI2_funciones.varias")
        xFunc.OpcionesUsuario xIdUsuario, xIdMenu, Toolbar1, xConeccion
        Set xFunc = Nothing
        '----------------------------------------------
...
Sub PreparaGrid()
    ...
    For A = LBound(xCamposVista) To UBound(xCamposVista)
        ...
        Db1.Columns(A).Width = NulosN(xCamposVista(A, 2))
...
Private Sub Toolbar1_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    If Button.Index = 3 Then
        ...
        If Rpta = vbYes Then
            ...
            xConeccion.Execute "DELETE * FROM var_edicion WHERE idmov = " & RstCab(xCampCla) & " AND idform = " & xIdMenu
...
RecorreControles 1
If RstCab.RecordCount = 0 Then Exit Sub
If RstCab.State = 0 Then Exit Sub
    For A = LBound(xCampos) To UBound(xCampos)
        If xCampos(A, 2) = "C" Or xCampos(A, 2) = "M" Then
...
Private Sub TxtTexto_Change(Index As Integer)
    If QueHace <> 3 Then
        If NulosC(TxtTexto(Index).Text) = "" Then
            LblDato(Index).Caption = ""
            LblIdCampo(Index).Caption = ""
        End If
    End If
End Sub
...
Function Grabar() As Boolean
    ...
    Dim xFunc As Object
    Set xFunc = CreateObject("SGI2_funciones.varias")
    xFunc.GrabarOperacion xIdUsuario, xIdMenu, QueHace, xHorIni, Time, Date, xConeccion, TxtTexto.Item(0).Text
    Set xFunc = Nothing
