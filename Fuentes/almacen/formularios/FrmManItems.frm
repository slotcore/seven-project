   ...
   Begin SizerOneLibCtl.TabOne TabOne1 
      ...
      Begin VB.Frame Frame2 
         ...
         Begin SizerOneLibCtl.TabOne TabOne2 
            ...
            Caption         =   "  Datos Producto | Datos Contables | Config. Adicional "
            Align           =   0
            CurrTab         =   1
            FirstTab        =   0
            Style           =   0
            ...
            Begin VB.Frame Frame7 
               ...
               Begin VB.Frame Frame8 
                  Caption         =   "Nombre Secundario Tipo Documento"
                  Height          =   1845
                  Left            =   90
                  TabIndex        =   117
                  Top             =   120
                  ...
                  Begin VSFlex7Ctl.VSFlexGrid fg 
                     ...
                     FormatString    =   $"FrmManItems.frx":2B10
                     ScrollTrack     =   0   'False
                     ScrollBars      =   3
                     ScrollTips      =   0   'False
                     MergeCells      =   0
            ...
            Begin VB.Frame Frame4 
               ...
               Begin VSFlex7Ctl.VSFlexGrid Fg1 
                  ...
                  ColWidthMax     =   0
                  ExtendLastCol   =   0   'False
                  FormatString    =   $"FrmManItems.frx":2CB6
                  ScrollTrack     =   0   'False
                  ScrollBars      =   3
                  ScrollTips      =   0   'False
                  MergeCells      =   0
            ...
            Begin VB.Frame Frame3 
               ...
               Begin VSFlex7Ctl.VSFlexGrid Fg2 
                  ...
                  ColWidthMax     =   0
                  ExtendLastCol   =   0   'False
                  FormatString    =   $"FrmManItems.frx":3944
                  ScrollTrack     =   0   'False
                  ScrollBars      =   3
                  ScrollTips      =   0   'False
                  MergeCells      =   0
...
Private Sub cmd_Click(Index As Integer)
    ...
    Select Case Index
        Case 0 ' Almacen
            ...
            nSQLId = GENERAR_SQL_ID(fg(0), fg(0).ColIndex("IDDOC"), " AND mae_documento.id", "NOT IN", True)
    
            cSQL = "SELECT mae_documento.id, mae_documento.descripcion " _
                + vbCr + "FROM mae_documento " _
                + vbCr + "WHERE mae_documento.id > 0 " & nSQLId
                        
            nTitulo = "Buscando Documento"
            ...
            fg(0).TextMatrix(fg(0).Rows - 1, fg(0).ColIndex("DESCRIPCION")) = NulosC(xRs("descripcion"))
            fg(0).TextMatrix(fg(0).Rows - 1, fg(0).ColIndex("IDDOC")) = NulosN(xRs("id"))
...
Function Grabar() As Boolean
    ...
    ' Se comenta por requerimiento del negocio
'    Preguntar = True
...
'    End If
    ...
    Dim RstDetItemDoc As New ADODB.Recordset
    Dim xId As Double
    Dim xIdItemDoc As Double
    
    On Error GoTo LaCague
    xCon.BeginTrans
    
    xIdItemDoc = HallaCodigoTabla("mae_itemdocconfig", xCon, "iditemdocconfig")
    If QueHace = 1 Then
        ...
        RST_Busq RstDetItemDoc, "SELECT * FROM mae_itemdocconfig", xCon
    ...
    Else
        ...
        xCon.Execute "DELETE * FROM mae_itemdocconfig WHERE iditem = " & xId & ""
        RST_Busq RstDetItemDoc, "SELECT * FROM mae_itemdocconfig", xCon
    End If
    ...
    ' Configuracion de segundo nombre Item
    If fg(0).Rows > 1 Then
        For A = 1 To fg(0).Rows - 1
            RstDetItemDoc.AddNew
            RstDetItemDoc("iditemdocconfig") = xIdItemDoc
            RstDetItemDoc("iditem") = xId
            RstDetItemDoc("iddoc") = fg(0).TextMatrix(A, fg(0).ColIndex("IDDOC"))
            RstDetItemDoc.Update
        Next A
    End If
...
Sub Eliminar()
    ...
    ' *****************************************
    ' Se verifica que no tenga movimientos
    RST_Busq xRs, "SELECT alm_ingresodet.iditem FROM alm_ingresodet WHERE (((alm_ingresodet.iditem) = " & RstPro("id") & "))", xCon
        
    If xRs.RecordCount > 0 Then
        MsgBox "No se puede eliminar el item seleccionado, cuenta con movimientos en almacén", vbInformation + vbOKOnly + vbDefaultButton1, xTitulo
        Set xRs = Nothing
        Exit Sub
    End If
    ' *****************************************
...
Sub MuestraSegundoTab()
    ...
    If NulosN(RstPro("idsubclas")) = 0 Then
        TxtIdSubClase.Text = ""
    Else
    ...
    ' Se carga configuracion Segundo Nombre
    Dim xRs As New ADODB.Recordset
    Dim B As Integer

    fg(0).Rows = fg(0).FixedRows
    RST_Busq xRs, "SELECT mae_itemdocconfig.iditemdocconfig, mae_itemdocconfig.iditem, mae_itemdocconfig.iddoc, mae_documento.descripcion " _
        + vbCr + "FROM mae_itemdocconfig INNER JOIN mae_documento ON mae_documento.id = mae_itemdocconfig.iddoc " _
        + vbCr + "WHERE ((mae_itemdocconfig.iditem)= " & RstPro("id") & ")", xCon
    
    If xRs.RecordCount <> 0 Then
        xRs.MoveFirst
        For B = 1 To xRs.RecordCount
            fg(0).Rows = fg(0).Rows + 1
            fg(0).TextMatrix(B, fg(0).ColIndex("IDDOC")) = NulosN(xRs("iddoc"))
            fg(0).TextMatrix(B, fg(0).ColIndex("DESCRIPCION")) = NulosC(xRs("descripcion"))
            xRs.MoveNext
