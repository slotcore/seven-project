...
Begin VB.Form FrmConsultaDiario 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CommandButton cmd_periodo 
         ...
         Index           =   0
         Left            =   1845
         Picture         =   "FrmConsultaDiario.frx":0382
         Style           =   1  'Graphical
         TabIndex        =   18
         Top             =   975
         Width           =   285
      ...
      Begin VB.CommandButton CmdBusProv 
         Height          =   230
         Left            =   4110
         Picture         =   "FrmConsultaDiario.frx":0704
         Style           =   1  'Graphical
         TabIndex        =   6
         Top             =   210
         Width           =   210
   ...
   Begin SizerOneLibCtl.TabOne TabOne1 
      ...
      Begin VB.Frame Frame3 
         ...
         Begin VSFlex7Ctl.VSFlexGrid Fg2 
            ...
            FormatString    =   $"FrmConsultaDiario.frx":3B24
            ScrollTrack     =   0   'False
            ScrollBars      =   3
            ScrollTips      =   0   'False
            MergeCells      =   0
      ...
      Begin VB.Frame Frame2 
         ...
         Begin VSFlex7Ctl.VSFlexGrid Fg1 
            ...
            ColWidthMax     =   0
            ExtendLastCol   =   0   'False
            FormatString    =   $"FrmConsultaDiario.frx":3BD6
            ScrollTrack     =   0   'False
            ScrollBars      =   3
            ScrollTips      =   0   'False
            MergeCells      =   0
...
'        ExportarExcelResumen
        xFun.VSFlexGrid_Exportar_MSExcel xCon, Fg2, "RESUMEN DEL DIARIO", "DEL " + TxtFchIni.Valor + " AL " + TxtFchFin.Valor, "Por Fecha", "diario.xls"  ', Rst, ""
        Set xFun = Nothing
    End If
End Sub
...
'                ProcesarDiario3 NulosN(LblIdLibro.Caption), "Libro " + Trim(Rst("descripcion") & "")
    ...
    Fg1.Rows = Fg1.Rows - 1
    If (xAcumulado(2, 0) <> 0 Or xAcumulado(2, 1) <> 0) And NulosC(TxtLibro.Text) = "" Then
        Fg1.Rows = Fg1.Rows + 2
...
'''    ProcesarResumen3 NulosN(LblIdLibro.Cmaption)
...
Private Sub ProcesarDiario3(IDLIBRO As Integer, N_Libro As String)
    ...
    If opt_fecha(0).Value = True Then '--por fecha
        If CDate(Me.TxtFchIni.Valor) = CDate("01/01/" + AnoTra) Then
            nSQLSaldo = " OR ( (con_diario.idlib = " & IDLIBRO & ") AND con_diario.fchasi IS NULL ) "
        Else
            nSQLSaldo = " AND (con_diario.fchasi) IS NOT NULL "
        End If
    Else '--por periodo
        If mMesIni = 1 Then
            nSQLSaldo = " OR ( (con_diario.idlib = " & IDLIBRO & ") AND con_diario.fchasi IS NULL ) "
    ...
    If opt_fecha(0).Value = True Then  '--por fecha
        nSQLWhere = " WHERE ( (con_diario.idlib = " & IDLIBRO & ") AND (con_diario.fchasi >=CDate('" & TxtFchIni.Valor & "') And con_diario.fchasi<=CDate('" + TxtFchFin.Valor + "')) ) "
    Else '--por intervalo
        nSQLWhere = " WHERE ( (con_diario.idlib = " & IDLIBRO & ") AND (con_diario.idmes >=  " & mMesIni & " AND con_diario.idmes <= " & mMesFin & ") ) "
    End If
    
    nSQLWhere = nSQLWhere & nSQLSaldo & " and (con_tc.idmon=2 or con_tc.idmon is null) "
    ...
    Case 4, 5
        '--4:Igv Percepciones
        '--5:Igv Retenciones
'        nSQL = "SELECT DISTINCT con_diario.idmov, IIf(con_diario.iddocpro=0,IIf(con_retencion.tipo=1,mae_prov.id,mae_cliente.id),IIf(con_retencion.tipo=1,mae_prov_1.id,mae_cliente_1.id)) AS idclipro, con_diario.idcue, IIf(con_diario.iddocpro=0,con_diario.idmon,IIf(con_retencion.tipo=1,mae_moneda_1.id,mae_moneda_2.id)) AS idmon, mae_libros.descripcion AS libdesc, IIf(con_diario.iddocpro=0,mae_documento.id,IIf(con_retencion.tipo=1,mae_documento_1.id,mae_documento_2.id)) AS tipdoc, " _
'            + vbCr + " IIf(con_diario.iddocpro=0,IIf(con_retencion.tipo=1,mae_prov.numruc,mae_cliente.numruc),IIf(con_retencion.tipo=1,mae_prov_1.numruc,mae_cliente_1.numruc)) AS numruc, IIf(con_diario.iddocpro=0,IIf(con_retencion.tipo=1,mae_prov.nombre,mae_cliente.nombre),IIf(con_retencion.tipo=1,mae_prov_1.nombre,mae_cliente_1.nombre)) AS apenom, " _
'            + vbCr + " IIf(con_diario.iddocpro=0,mae_documento.abrev,IIf(con_retencion.tipo=1,mae_documento_1.abrev,mae_documento_2.abrev)) AS tdocdesc, IIf(con_diario.iddocpro=0,IIf(con_retencion.tipo=1,mae_documento.codsun,mae_documento.codsun),IIf(con_retencion.tipo=1,mae_documento_1.codsun,mae_documento_2.codsun)) AS tdocsun, " _
'            + vbCr + " IIf(con_diario.iddocpro=0,con_retencion.fchemi,IIf(con_retencion.tipo=1,com_compras.fchdoc,vta_ventas.fchdoc)) AS fchdoc, con_tc.impven AS tc, IIf(con_diario.iddocpro=0,IIf(con_retencion!numser Is Null Or con_retencion!numser='','',con_retencion!numser & '-') & con_retencion!numdoc,IIf(con_retencion.tipo=1,IIf(com_compras!numser Is Null Or com_compras!numser='','',com_compras!numser & '-') & com_compras!numdoc,IIf(vta_ventas!numser Is Null Or vta_ventas!numser='','',vta_ventas!numser & '-') & vta_ventas!numdoc)) AS numdoc, " _
'            + vbCr + " Left(con_retencion.numreg,2) & Format(mae_libros.codsun,'00') & Right(con_retencion.numreg,4) AS registro, IIf(con_diario.iddocpro=0,Left(con_retencion.numreg,2) & Format(mae_libros.codsun,'00') & Right(con_retencion.numreg,4),IIf(con_retencion.tipo=1,Left(com_compras.numreg,2) & Format(mae_libros_1.codsun,'00') & Right(com_compras.numreg,4),Left(vta_ventas.numreg,2) & Format(mae_libros_2.codsun,'00') & Right(vta_ventas.numreg,4))) AS registroref, " _
'            + vbCr + " con_retencion.fchemi AS fchope,con_retencion.glosa, IIf(con_diario.iddocpro=0,mae_libros.codsun,IIf(con_retencion.tipo=1,mae_libros_1.codsun,mae_libros_2.codsun)) AS libsun, IIf(con_diario.iddocpro=0,CDbl(con_diario.numasi),IIf(con_retencion.tipo=1,IIf(com_compras.numreg Is Null,0,CDbl(Right(com_compras.numreg,4))), " _
'            + vbCr + " IIf(vta_ventas.numreg Is Null,0,CDbl(Right(vta_ventas.numreg,4))))) AS corr, " _
'            + vbCr + " IIf(con_diario.iddocpro=0,IIf(con_retencion!numser Is Null Or con_retencion!numser='','',con_retencion!numser & '-') & con_retencion!numdoc,IIf(con_retencion.tipo=1,IIf(com_compras!numser Is Null Or com_compras!numser='','',com_compras!numser & '-') & com_compras!numdoc,IIf(vta_ventas!numser Is Null Or vta_ventas!numser='','',vta_ventas!numser & '-') & vta_ventas!numdoc)) AS docsustenta, " _
'            + vbCr + " con_planctas.cuenta AS ctanum, con_planctas.descripcion AS ctadesc, IIf(con_diario.iddocpro=0,mae_moneda.simbolo,IIf(con_retencion.tipo=1,mae_moneda_1.simbolo,mae_moneda_2.simbolo)) AS simbolo, " _
'            + vbCr + " IIf(con_diario.idmon=2,IIf(con_tc.impven Is Null,0,con_diario.impdebdol*con_tc.impven),con_diario.impdebsol) AS impdebesol, " _
'            + vbCr + " IIf(con_diario.idmon=2,IIf(con_tc.impven Is Null,0,con_diario.imphabdol*con_tc.impven),con_diario.imphabsol) AS imphabersol, " _
'            + vbCr + " IIf(con_diario.idmon=2,con_diario.impdebdol,IIf(con_tc.impven Is Null Or con_diario.impdebsol=0,0,(con_diario.impdebsol/con_tc.impven))) AS impdebedol, " _
'            + vbCr + " IIf(con_diario.idmon=2,con_diario.imphabdol,IIf(con_tc.impven Is Null Or con_diario.imphabsol=0,0,(con_diario.imphabsol/con_tc.impven))) AS imphaberdol, " _
'            + vbCr + " '' AS RefTDoc, '' AS RefNumDoc " _
'            + vbCr + " FROM ((((((((((mae_documento RIGHT JOIN ((((((con_planctas RIGHT JOIN con_diario ON con_planctas.id = con_diario.idcue) LEFT JOIN mae_moneda ON con_diario.idmon = mae_moneda.id) LEFT JOIN con_tc ON con_diario.fchdoc = con_tc.fecha) LEFT JOIN com_compras ON con_diario.iddocpro = com_compras.id) LEFT JOIN vta_ventas ON con_diario.iddocpro = vta_ventas.id) RIGHT JOIN (con_retencion LEFT JOIN mae_libros ON con_retencion.idlib = mae_libros.id) ON con_diario.idmov = con_retencion.id) ON mae_documento.id = con_retencion.iddoc) LEFT JOIN mae_moneda AS mae_moneda_1 ON com_compras.idmon = mae_moneda_1.id) LEFT JOIN mae_moneda AS mae_moneda_2 ON vta_ventas.idmon = mae_moneda_2.id) " _
'            + vbCr + " LEFT JOIN mae_documento AS mae_documento_1 ON com_compras.tipdoc = mae_documento_1.id) LEFT JOIN mae_documento AS mae_documento_2 ON vta_ventas.tipdoc = mae_documento_2.id) LEFT JOIN mae_libros AS mae_libros_1 ON com_compras.idlib = mae_libros_1.id) LEFT JOIN mae_libros AS mae_libros_2 ON vta_ventas.idlib = mae_libros_2.id) LEFT JOIN mae_cliente ON con_retencion.idpro = mae_cliente.id) LEFT JOIN mae_prov ON con_retencion.idpro = mae_prov.id) LEFT JOIN mae_prov AS mae_prov_1 ON com_compras.idpro = mae_prov_1.id) LEFT JOIN mae_cliente AS mae_cliente_1 ON vta_ventas.idcli = mae_cliente_1.id "
...
'    Case 39 '--Rendición de Cuentas
    ...
    If IDLIBRO <> 4 And IDLIBRO <> 5 Then nSQL = nSQL + vbCr + nSQLWhere
    '--------
...
Private Sub ProcesarResumen3(IDLIBRO As Integer)
    ...
    If opt_fecha(0).Value = True Then '--por fecha
        nSQLWhere = " ( " & nSQLLibro & " con_diario.fchasi between CDate('" & TxtFchIni.Valor & "') and cdate('" & TxtFchFin.Valor & "') ) "
        If CDate(Me.TxtFchIni.Valor) = CDate("01/01/" + AnoTra) Then
            nSQLWhere = nSQLWhere & " or ( " & nSQLLibro & " con_diario.fchasi IS NULL ) "
        Else
            nSQLWhere = nSQLWhere & " and con_diario.fchasi IS NOT NULL "
        End If
    Else '--por intervalo
        nSQLWhere = " ( " & nSQLLibro & " month(con_diario.fchasi) >= " & mMesIni & " and month(con_diario.fchasi)<= " & mMesFin & " ) "
