   ...
   Begin MSComctlLib.Toolbar Toolbar1 
      ...
      BeginProperty Buttons {66833FE8-8583-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty Button13 {66833FEA-8583-11D1-B16A-00C0F0283628} 
            Object.ToolTipText     =   "Imprimir Voucher"
            ImageIndex      =   12
         EndProperty
...
Function Grabar() As Boolean
    ...
    For A = 1 To Fg2.Rows - 1
        If NulosN(Fg2.TextMatrix(A, 3)) = 0 Then
            MsgBox "No ha especificado el destino del ingreso", vbInformation + vbOKOnly + vbDefaultButton1
            Grabar = False
            Exit Function
        End If
        If NulosN(Fg2.TextMatrix(A, 7)) = 0 Or NulosN(Fg2.TextMatrix(A, 7)) = 0 Then
            MsgBox "No ha especificado el importe para el destino del ingreso", vbInformation + vbOKOnly + vbDefaultButton1
            Grabar = False
            Exit Function
        End If
    Next A
    ...
    For A = 1 To Fg1.Rows - 1
        ...
        If NulosN(Fg1.TextMatrix(A, 5)) = 6 Then   'grabamos los datos del cheque
            ...
            If RstTmpDocOri.RecordCount <> 0 Then
                ...
                RstDet1_1("numdoc") = Trim(RstTmpDocOri("numdoc"))
        ...
        Select Case xModulo
            Case 1, 9, 10, 2, 4, 11, 19
                '--Canjes de Documentos
                '--1=Compras; 9=Honorarios, 10=Reembolsables
...
''''    If IdModulo = 2 Or IdModulo = 8 Or IdModulo = 4 Or IdModulo = 11 Or IdModulo = 19 Then
''''        Agregando = True
''''        If mVentanaActiva = 0 Then
''''            If TxtIdMon.Text = "1" Then
''''                Fg1.TextMatrix(Fg1.Row, 7) = Format(TxtTotal4.Text, FORMAT_MONTO)
''''                Fg1.TextMatrix(Fg1.Row, 8) = NulosN(TxtTotal4.Text) / NulosN(Fg1.TextMatrix(Fg1.Row, 2))
''''                Fg1.TextMatrix(Fg1.Row, 8) = Format(Fg1.TextMatrix(Fg1.Row, 8), FORMAT_MONTO)
''''            Else
''''                Fg1.TextMatrix(Fg1.Row, 8) = Format(TxtTotal4.Text, FORMAT_MONTO)
''''                Fg1.TextMatrix(Fg1.Row, 7) = NulosN(TxtTotal4.Text) * NulosN(Fg1.TextMatrix(Fg1.Row, 2))
''''                Fg1.TextMatrix(Fg1.Row, 7) = Format(Fg1.TextMatrix(Fg1.Row, 7), FORMAT_MONTO)
''''            End If
''''
''''            TotalizarFG1
    ...
    For A = 1 To Fg1.Rows - 1
        ...
        If RstTmpDocOri.RecordCount <> 0 Then
            ...
            For B = 1 To RstTmpDocOri.RecordCount
                ...
                If TxtIdMon.Text = "1" Then
                    Fg4.TextMatrix(Fg4.Rows - 1, 6) = Format(RstTmpDocOri("imptot"), FORMAT_MONTO)
                ...
                Else
                    Fg4.TextMatrix(Fg4.Rows - 1, 8) = Format(RstTmpDocOri("imptot"), FORMAT_MONTO)
        ...
        Else
            ...
            If TxtIdMon.Text = "1" Then
                Fg4.TextMatrix(Fg4.Rows - 1, 6) = Format(NulosN(Fg1.TextMatrix(A, 7)), FORMAT_MONTO)
                Fg4.TextMatrix(Fg4.Rows - 1, 8) = NulosN(Fg1.TextMatrix(A, 7)) / NulosN(Fg1.TextMatrix(A, 2))
                Fg4.TextMatrix(Fg4.Rows - 1, 8) = Format(Fg4.TextMatrix(Fg4.Rows - 1, 8), FORMAT_MONTO)
            Else
                Fg4.TextMatrix(Fg4.Rows - 1, 8) = Format(NulosN(Fg1.TextMatrix(A, 8)), FORMAT_MONTO)
...
'    TxtHabDol.Text = Format(TotHabDol, FORMAT_MONTO)
    ...
    TxtDebSol.Text = Format(GRID_SUMAR_COL(Fg4, 6), FORMAT_MONTO)
    TxtHabSol.Text = Format(GRID_SUMAR_COL(Fg4, 7), FORMAT_MONTO)
    TxtDebDol.Text = Format(GRID_SUMAR_COL(Fg4, 8), FORMAT_MONTO)
    TxtHabDol.Text = Format(GRID_SUMAR_COL(Fg4, 9), FORMAT_MONTO)
...
Private Sub Command8_Click()
    ...
    RstTmpDocOri.Filter = "xtipo=" & mVentanaActiva & " AND idconc = " & NulosN(Fg6.TextMatrix(Fg6.Row, 11)) & " AND iddocu = " & NulosN(Fg6.TextMatrix(Fg6.Row, 12)) & ""
    ...
    RstTmpDocOri.Filter = adFilterNone
    RstTmpDocOri.Filter = "idconc = " & NulosN(Fg6.TextMatrix(Fg6.Row, 11)) & ""
...
Private Sub Command9_Click()
    ...
    If Fg6.Row < 1 Then
        MsgBox "No ha especificado la información necesaria de la operación", vbInformation, xTitulo
        Fg6.Rows = Fg6.Rows + 1
        Fg6.Row = 1
        Fg6.SetFocus
        Exit Sub
...
'        xForm.SQLCad = "SELECT con_planctas.cuenta, con_planctas.descripcion AS descuen, tes_origen.id, tes_origen.idmon, tes_origen.descripcion, tes_origen.idcuen, " _
        ...
        & " WHERE (((tes_origen.idmon)=" & NulosN(TxtIdMon.Text) & ") AND ((tes_origen.tipmov)=1) AND ((tes_origen.activo)=-1)) " & nSQLId
...
'        xForm.SQLCad = "SELECT con_planctas.cuenta, con_planctas.descripcion AS descuen, tes_destino.id, tes_destino.idmon, tes_destino.descripcion, tes_destino.idcuen, " _
        ...
        & " WHERE (((tes_destino.idmon)=" & NulosN(TxtIdMon.Text) & ") AND ((tes_destino.tipmov)=1) AND ((tes_destino.activo)=-1)) " & nSQLId
    ...
    If Col = 7 Or Col = 8 Then
        ...
        If IdModulo = 1 Or IdModulo = 8 Or IdModulo = 9 Or IdModulo = 10 Or _
           ...
           IdModulo = 20 Then
            '--En caso de devoluciones por pago en exceso o cobranza de nc de compra
            '--1=Compras
            '--8=Boleta de Pago
            '--9=Honorarios
            '--10=Reembolsables
...
Private Sub Fg3_CellChanged(ByVal Row As Long, ByVal Col As Long)
    ...
    '**********************************************************
        ...
        If Col = 7 Then
            ...
            If RstTMPDoc.RecordCount <> 0 Then
                RstTMPDoc.MoveFirst
                RstTMPDoc.Filter = "xtipo=" & mVentanaActiva & " AND idconc = " & IdConcepto & " AND corr= " & NulosN(Fg3.TextMatrix(Row, 15)) & ""
    ...
    If Col = 9 Then
        ...
        RstTMPDoc.Filter = adFilterNone
        RstTMPDoc.Filter = "xtipo=" & mVentanaActiva & " AND idconc = " & IdConcepto & " AND corr = " & Fg3.TextMatrix(Fg3.Row, 15) & ""
...
Sub PreparaRSTOri()
    ...
    xCampos(4, 0) = "numdoc":         xCampos(4, 1) = "C":      xCampos(4, 2) = "30" ' numero de documento
    xCampos(5, 0) = "imptot":         xCampos(5, 1) = "D":      xCampos(5, 2) = "2" ' importe total del documento
    xCampos(6, 0) = "impsal":         xCampos(6, 1) = "D":      xCampos(6, 2) = "2" ' saldo del documento
    xCampos(7, 0) = "impsal2":        xCampos(7, 1) = "D":      xCampos(7, 2) = "2" ' saldo del documento en la moneda de trabajo
    xCampos(8, 0) = "acuent":         xCampos(8, 1) = "D":      xCampos(8, 2) = "2" ' importe acuenta
...
Private Sub Fg3_EnterCell()
    ...
    If Fg3.Col = 10 Or Fg3.Col = 9 Or Fg3.Col = 7 Then
        Fg3.Editable = flexEDKbdMouse
    Else
        Fg3.Editable = flexEDNone
    End If
...
Sub MuestraSegundoTab()
    ...
    'mostramos el destino del movimiento "HABER"
    ...
    If rst.RecordCount <> 0 Then
        ...
        For A = 1 To rst.RecordCount
            ...
            Select Case xModulo
                ...
                Case 8 '-- boleta pago
                    nSQL = "SELECT [pla_boleta]![numser]+'-'+[pla_boleta]![numdoc] AS numdoc, UCase([pla_empleados]![apepat])+' '+UCase([pla_empleados]![apemat])+', '+[pla_empleados]![nom] AS apenom, " _
                        ...
                        & " tes_cajadestinodet.idper, tes_cajadestinodet.saldo, tes_cajadestinodet.acuenta, pla_boleta.fchdoc, pla_boleta.idmon, pla_boleta.id, pla_boleta.imptot, tes_cajadestinodet.idori " _
    ...
    'mostramos el origen del movimiento "DEBE"
    Set rst = Nothing
    nSQL = "SELECT con_planctas.cuenta, con_planctas.descripcion AS descuen, tes_origen.id, tes_origen.idmon, tes_origen.descripcion, tes_origen.idcuen, " _
        & " tes_origen.tipmov, tes_origen.idmod, tes_origen.entgen, 0 AS numdocasi, tes_cajaori.importe, tes_cajaori.tc,tes_origen.idbcocta " _
        & " FROM (tes_origen LEFT JOIN con_planctas ON tes_origen.idcuen = con_planctas.id) RIGHT JOIN tes_cajaori ON tes_origen.id = tes_cajaori.idori " _
        & " WHERE (((tes_cajaori.idtes)=" & xId & "))"
                ...
                If Rst2.RecordCount <> 0 Then
                    ...
                    For x = 1 To Rst2.RecordCount
                        ...
                        Else
                            If NulosN(TxtIdMon.Text) = 1 Then
                                If Rst2("idmon") = 2 Then
                                    RstTMPDoc("impsal2") = NulosN(Rst2("saldo")) * NulosN(Fg1.TextMatrix(A, 2))
                                End If
                            Else
                                If Rst2("idmon") = 1 Then
                                    RstTMPDoc("impsal2") = NulosN(Rst2("saldo")) / NulosN(Fg1.TextMatrix(A, 2))
...
Private Sub Toolbar1_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    If Button.Index = 13 Then
        ...
        If TabOne1.CurrTab = 1 And QueHace = 3 Then
            ImprimirOperacion 1, RstMov("id"), RstMov("fchreg"), xCon
        Else
            ImprimirOperacion 1, 0, RstMov("fchreg"), xCon
        End If
    End If
...
Function ExisteDocumento(IdConcepto As Integer, IdDocumento As Double)
    Dim A As Integer
...
Sub CargarDocumentos_XXX(IdProveedor As Integer)
...
'    If xCadWhere1 <> "" Then xCadWhere1 = " AND " & " ( " & xCadWhere1 & ")"
    ...
    xCampos(0, 0) = "Nº Documento":  xCampos(0, 1) = "numdoc":         xCampos(0, 2) = "1500":    xCampos(0, 3) = "C":    xCampos(0, 4) = "S"
    xCampos(1, 0) = "T.D.":          xCampos(1, 1) = "abrev":          xCampos(1, 2) = "500":     xCampos(1, 3) = "C":    xCampos(1, 4) = "N"
    ...
    xCampos(5, 0) = "Cliente":       xCampos(5, 1) = "nombre":         xCampos(5, 2) = "3500":    xCampos(5, 3) = "C":    xCampos(5, 4) = "N"
    xCampos(6, 0) = "Moneda":        xCampos(6, 1) = "simbolo":        xCampos(6, 2) = "800":     xCampos(6, 3) = "C":    xCampos(6, 4) = "N"
    xCampos(7, 0) = "Condicion":     xCampos(7, 1) = "desccond":       xCampos(7, 2) = "1500":    xCampos(7, 3) = "C":    xCampos(7, 4) = "N"
    ...
    If IdModulo = 2 Then
        ...
        If IdProveedor = 0 Then
            nSQL = "SELECT 0 AS xSel, vta_ventas.id, mae_cliente.nombre, mae_documento.abrev, mae_documento.codsun, vta_ventas.fchdoc, vta_ventas.fchven, " _
                ...
                & " vta_ventas.impsal, vta_ventas.tipdoc, vta_ventas.idmon, mae_condpago.descripcion AS desccond, Format([fchdoc],'dd') AS dia, Format([fchdoc],'mm') AS mes, Format([fchdoc],'yy') AS ano" _
                ...
                & " Where (((vta_ventas.impsal) <> 0)) ORDER BY [vta_ventas]![numser]+'-'+[vta_ventas]![numdoc]"
        Else
            nSQL = "SELECT 0 AS xSel, vta_ventas.id, mae_cliente.nombre, mae_documento.abrev, mae_documento.codsun, vta_ventas.fchdoc, vta_ventas.fchven, " _
                & " [vta_ventas]![numser]+'-'+[vta_ventas]![numdoc] AS numdoc, mae_moneda.simbolo, vta_ventas.imptotdoc AS imptot, 'Ventas' AS origen, " _
                & " 2 AS idori, vta_ventas.impsal, vta_ventas.tipdoc, vta_ventas.idmon, mae_condpago.descripcion AS desccond, Format([fchdoc],'dd') AS dia, Format([fchdoc],'mm') AS mes, Format([fchdoc],'yy') AS ano " _
                & " FROM (((vta_ventas LEFT JOIN mae_documento ON vta_ventas.tipdoc = mae_documento.id) LEFT JOIN mae_moneda ON vta_ventas.idmon = mae_moneda.id) " _
                & " LEFT JOIN mae_cliente ON vta_ventas.idcli = mae_cliente.id) LEFT JOIN mae_condpago ON vta_ventas.idconpag = mae_condpago.id " _
                & " Where (((vta_ventas.impsal) <> 0) And ((vta_ventas.idcli) = " & IdProveedor & ")) ORDER BY [vta_ventas]![numser]+'-'+[vta_ventas]![numdoc]"
    ...
    ElseIf IdModulo = 4 Then
        ...
        If IdProveedor = 0 Then
            nSQL = "SELECT 0 AS xSel,  let_letradet.corr AS id, mae_cliente.nombre, mae_documento.abrev, mae_documento.codsun, let_letradet.fchemi AS fchdoc, let_letradet.fchven, [let_letra].[ano] & ' ' & [let_letradet].[numdoc] & ' ' & [let_letradet].[numser] AS numdoc, mae_moneda.simbolo, let_letradet.implet AS imptot, 'LETRA' AS origen, 4 AS idori, let_letradet.impsal, let_letra.tipdoc, let_letra.idmon, '' AS desccond, Format([fchdoc],'dd') AS dia, Format([fchdoc],'mm') AS mes, Format([fchdoc],'yy') AS ano  " _
        ...
        Else
            nSQL = "SELECT 0 AS xSel,  let_letradet.corr AS id, mae_cliente.nombre, mae_documento.abrev, mae_documento.codsun, let_letradet.fchemi AS fchdoc, let_letradet.fchven, [let_letra].[ano] & ' ' & [let_letradet].[numdoc] & ' ' & [let_letradet].[numser] AS numdoc, mae_moneda.simbolo, let_letradet.implet AS imptot, 'LETRA' AS origen, 4 AS idori, let_letradet.impsal, let_letra.tipdoc, let_letra.idmon, '' AS desccond, Format([fchdoc],'dd') AS dia, Format([fchdoc],'mm') AS mes, Format([fchdoc],'yy') AS ano  " _
                + vbCr + " FROM mae_moneda RIGHT JOIN (mae_documento RIGHT JOIN ((let_letra LEFT JOIN mae_cliente ON let_letra.idclipro = mae_cliente.id) RIGHT JOIN let_letradet ON let_letra.id = let_letradet.idlet) ON mae_documento.id = let_letra.tipdoc) ON mae_moneda.id = let_letra.idmon " _
                + vbCr + " WHERE let_letra.idclipro=" & IdProveedor & "  "
...
Sub CargarDocumentos(IdProveedor As Integer)
    ...
    '             Mostrar lista de liquidacion Gasto Debito
    '             01/09/09 Por johan Castro
    '             Mostrar Lista de Planilla de letras enviadas al Banco, Letras
    '             24/11/11 Por johan Castro
    '             Quitar filtro x tipo de documento
    '             Agregar codigo para mostrar selección de registro (compras, reembolsables, honorarios)
    '             01/02/12
...
Private Sub CtaCte(xId As Double)
'--cuenta corriente savar
'--x revisar
    Dim RstTes As New ADODB.Recordset
    Dim RstTesdes As New ADODB.Recordset
    ...
    Set RstTes = Nothing
    Set RstTesdes = Nothing
    Set RstTesDesDet = Nothing
...
Sub CargarDocumentos_xxxxx(IdProveedor As Integer)
...
'            If xCadWhere1 <> "" Then xCadWhere1 = " AND (" & xCadWhere1 & ")"
            ...
            If IdProveedor = 0 Then '--mostrar todos los documentos
                    
                nSQL = "SELECT 0 AS xSel, com_compras.id, mae_prov.nombre, mae_documento.abrev, mae_documento.codsun, com_compras.fchdoc, " _
                    & " Format(com_compras.fchdoc,'mm/dd/yy') AS fchdoc1, com_compras.fchven, [com_compras]![numser]+'-'+[com_compras]![numdoc] AS numdoc, mae_moneda.simbolo, " _
                    & " com_compras.imptot, 'Compras' AS origen, 1 AS idori, com_compras.impsal, com_compras.idmon, com_compras.tipdoc, mae_condpago.descripcion AS desccond, " _
                    & " Format([fchdoc],'dd') AS dia, Format([fchdoc],'mm') AS mes, Format([fchdoc],'yy') AS ano " _
                    & " FROM mae_condpago INNER JOIN (mae_documento RIGHT JOIN (mae_moneda RIGHT JOIN (mae_prov RIGHT JOIN com_compras ON mae_prov.id = com_compras.idpro) " _
                    & " ON mae_moneda.id = com_compras.idmon) ON mae_documento.id = com_compras.tipdoc) ON mae_condpago.id = com_compras.idconpag " _
                    & " WHERE (((com_compras.impsal)<>0) " & xCadWhere1 & ")" & nSQLNotIn _
                    & " Union " _
                    & " SELECT 0 AS xSel, con_percepcion.id, mae_prov.nombre, mae_documento.abrev, mae_documento.codsun, con_percepcion.fchdoc, " _
                    & " Format(con_percepcion.fchdoc,'mm/dd/yy') AS fchdoc1, '' AS fchven, [con_percepcion]![numser]+'-'+[con_percepcion]![numdoc] AS numdoc, " _
                    & " mae_moneda.simbolo, con_percepcion.imptotper AS imptot, 'Percepcion' AS origen, 2 AS idori, con_percepcion.impsal, con_percepcion.idmon, " _
                    & " con_percepcion.tipdoc, '' AS desccond, Format([fchdoc],'dd') AS dia, Format([fchdoc],'mm') AS mes, Format([fchdoc],'yy') AS ano " _
                    & " FROM ((con_percepcion LEFT JOIN mae_documento ON con_percepcion.tipdoc = mae_documento.id) LEFT JOIN mae_moneda ON con_percepcion.idmon = mae_moneda.id) " _
                    & " LEFT JOIN mae_prov ON con_percepcion.idcli = mae_prov.id " _
            ...
            Else '--mostrar documentos de un proveedor(seleccionado por usuario)
                    
                nSQL = "SELECT 0 as xSel,  com_compras.id, mae_prov.nombre, mae_documento.abrev,mae_documento.codsun, com_compras.fchdoc,format(com_compras.fchdoc,'mm/dd/yy') as fchdoc1, com_compras.fchven, [com_compras]![numser]+'-'+[com_compras]![numdoc] AS numdoc, " _
                    & " mae_moneda.simbolo, com_compras.imptot, 'Compras' AS origen, 1 AS idori, com_compras.impsal, com_compras.idmon, com_compras.tipdoc, " _
                    & " Format([fchdoc],'dd') AS dia, Format([fchdoc],'mm') AS mes, Format([fchdoc],'yy') AS ano  " _
                    & " FROM mae_prov RIGHT JOIN (mae_moneda RIGHT JOIN " _
                    & " (mae_documento RIGHT JOIN com_compras ON mae_documento.id = com_compras.tipdoc) ON mae_moneda.id = com_compras.idmon) ON mae_prov.id = com_compras.idpro " _
                    & " WHERE (((com_compras.impsal)<>0) AND ((com_compras.idpro)=" & IdProveedor & ") " & xCadWhere1 & ")" & nSQLNotIn _
                    & " Union " _
                    & " SELECT 0 as xSel,con_percepcion.id, mae_prov.nombre, mae_documento.abrev,mae_documento.codsun, con_percepcion.fchdoc, format(con_percepcion.fchdoc,'mm/dd/yy') as fchdoc1,'' AS fchven, [con_percepcion]![numser]+'-'+[con_percepcion]![numdoc] AS numdoc, " _
                    & " mae_moneda.simbolo, con_percepcion.imptotper AS imptot, 'Percepcion' AS origen, 2 AS idori, con_percepcion.impsal, con_percepcion.idmon, con_percepcion.tipdoc, " _
                    & " Format([fchdoc],'dd') AS dia, Format([fchdoc],'mm') AS mes, Format([fchdoc],'yy') AS ano " _
                    & " FROM ((con_percepcion LEFT JOIN " _
                    & " mae_documento ON con_percepcion.tipdoc = mae_documento.id) LEFT JOIN mae_moneda ON con_percepcion.idmon = mae_moneda.id) LEFT JOIN mae_prov " _
                    & " ON con_percepcion.idcli = mae_prov.id Where (((con_percepcion.impsal) <> 0) And ((con_percepcion.idcli) = " & IdProveedor & "))"
        ...
        Case 2  '--ventas
            '--sentencia sql para no mostrar documentos seleccionados
            nSQLNotIn = GRID_GENERAR_SQL_ID(Fg3, 12, " AND vta_ventas.id", " NOT IN", True)
            ...
            If IdProveedor = 0 Then
                nSQL = "SELECT 0 AS xSel, vta_ventas.id, mae_cliente.nombre, mae_documento.abrev, mae_documento.codsun, vta_ventas.fchdoc, vta_ventas.fchven, " _
                    ...
                    & " FROM (((vta_ventas LEFT JOIN mae_documento ON vta_ventas.tipdoc = mae_documento.id) LEFT JOIN mae_moneda ON vta_ventas.idmon = mae_moneda.id) " _
                    & " LEFT JOIN mae_cliente ON vta_ventas.idcli = mae_cliente.id) LEFT JOIN mae_condpago ON vta_ventas.idconpag = mae_condpago.id " _
                    & " Where (((vta_ventas.impsal) <> 0)) " & nSQLNotIn & " ORDER BY vta_ventas!numser+'-'+vta_ventas!numdoc"
            Else
                nSQL = "SELECT 0 AS xSel, vta_ventas.id, mae_cliente.nombre, mae_documento.abrev, mae_documento.codsun, vta_ventas.fchdoc, vta_ventas.fchven, " _
                    & " vta_ventas!numser+'-'+vta_ventas!numdoc AS numdoc, mae_moneda.simbolo, vta_ventas.imptotdoc AS imptot, 'Ventas' AS origen, " _
                    & " 2 AS idori, vta_ventas.impsal, vta_ventas.tipdoc, vta_ventas.idmon, mae_condpago.abrev AS desccond, Format(fchdoc,'dd') AS dia, Format(fchdoc,'mm') AS mes, Format(fchdoc,'yy') AS ano " _
                    & " FROM (((vta_ventas LEFT JOIN mae_documento ON vta_ventas.tipdoc = mae_documento.id) LEFT JOIN mae_moneda ON vta_ventas.idmon = mae_moneda.id) " _
                    & " LEFT JOIN mae_cliente ON vta_ventas.idcli = mae_cliente.id) LEFT JOIN mae_condpago ON vta_ventas.idconpag = mae_condpago.id " _
                    & " Where (((vta_ventas.impsal) <> 0) And ((vta_ventas.idcli) = " & IdProveedor & ")) " & nSQLNotIn & " ORDER BY vta_ventas!numser+'-'+vta_ventas!numdoc"
    
            End If
            nTitulo = "Buscando Documentos del Venta"
            xCampos(5, 0) = "Cliente":
'-------
            
            
        Case 4 '--letras
            '--sentencia sql para mostrar documentos de un cliente
            If IdProveedor <> 0 Then nSQLIdPer = " WHERE let_letra.idclipro=" & IdProveedor & " "
            '--sentencia sql para no mostrar documentos seleccionados
            nSQLNotIn = nSQLIdPer & GRID_GENERAR_SQL_ID(Fg3, 12, " AND let_letradet.corr", " NOT IN", True)
            
            '--dar consitencia al filtro
            If InStr(nSQLNotIn, "WHERE") = 0 Then nSQLNotIn = Replace(nSQLNotIn, "AND", "WHERE")
                        
            '--se mostraran todas las letras
                
            nSQL = "SELECT 0 as xSel, let_letradet.corr AS id, mae_cliente.nombre, mae_documento.abrev, mae_documento.codsun, let_letradet.fchemi AS fchdoc, " _
                + vbCr + " format(let_letradet.fchemi,'mm/dd/yy') as fchdoc1, let_letradet.fchven, [let_letra].[ano] & ' ' & [let_letradet].[numdoc] & ' ' & [let_letradet].[numser] AS numdoc, " _
                + vbCr + " mae_moneda.simbolo, let_letradet.implet AS imptot, 'LETRA' AS origen, 0 AS idori, let_letradet.impsal, let_letra.tipdoc, let_letra.idmon, '' AS desccond, Format([fchdoc],'dd') AS dia, Format([fchdoc],'mm') AS mes, Format([fchdoc],'yy') AS ano  " _
                + vbCr + " FROM mae_moneda RIGHT JOIN (mae_documento RIGHT JOIN ((let_letra LEFT JOIN mae_cliente ON let_letra.idclipro = mae_cliente.id) RIGHT JOIN let_letradet ON let_letra.id = let_letradet.idlet) ON mae_documento.id = let_letra.tipdoc) ON mae_moneda.id = let_letra.idmon " _
                + vbCr + nSQLNotIn
        ...
        Case 8 '--planillas
            '--sentencia sql para no mostrar documentos seleccionados
            nSQLNotIn = GRID_GENERAR_SQL_ID(Fg3, 12, " AND pla_boleta.id", " NOT IN", True)
        
            If IdProveedor = 0 Then '--mostrar todos los empleados
            
            Else '--mostrar documentos de un empleado(seleccionado por usuario)
            
            End If
            nSQL = "SELECT 0 as xSel, pla_boleta.id, con_meses.descripcion, UCase(pla_empleados!apepat)+' '+UCase(pla_empleados!apemat)+', '+pla_empleados!nom AS nombre, " _
                & " pla_boleta.ano, mae_documento.abrev, mae_moneda.simbolo, pla_boleta.imptot, pla_boleta.impsal, [pla_boleta]![numser]+'-'+[pla_boleta]![numdoc] AS numdoc, " _
                & " pla_boleta.fchdoc,format(pla_boleta.fchdoc,'mm/dd/yy') as fchdoc1, pla_boleta.iddoc as tipdoc, pla_boleta.idmon,0 as idori, '' AS desccond, Format([fchdoc],'dd') AS dia, Format([fchdoc],'mm') AS mes, Format([fchdoc],'yy') AS ano  " _
                & " FROM pla_empleados RIGHT JOIN (((pla_boleta LEFT JOIN con_meses ON pla_boleta.idmes = con_meses.id) " _
                & " LEFT JOIN mae_documento ON pla_boleta.iddoc = mae_documento.id) LEFT JOIN mae_moneda ON pla_boleta.idmon = mae_moneda.id) ON pla_empleados.id = pla_boleta.idemp " _
                & " Where (((pla_boleta.impsal) <> 0)) " & nSQLNotIn & " ORDER BY con_meses.descripcion, UCase(pla_empleados!apepat)+' '+UCase(pla_empleados!apemat)+', '+pla_empleados!nom"
        ...
        Case 9 '--honorarios
            '--sentencia sql para mostrar documentos de un prestador de servicio
            If IdProveedor <> 0 Then nSQLIdPer = " AND com_honorarios.idpro=" & IdProveedor & " "
            '--sentencia sql para no mostrar documentos seleccionados
            nSQLNotIn = GRID_GENERAR_SQL_ID(Fg3, 12, " AND com_honorarios.id", " NOT IN", True)
            
            nSQL = "SELECT 0 as xSel, com_honorarios.id, mae_prov.nombre, mae_documento.abrev,mae_documento.codsun, com_honorarios.fchdoc, " _
                + vbCr + " format(com_honorarios.fchdoc,'mm/dd/yy') as fchdoc1, com_honorarios.fchven, IIf([com_honorarios]![numser] Is Null,'', " _
                + vbCr + " [com_honorarios]![numser] & '-') & [com_honorarios]![numdoc]  AS numdoc, mae_moneda.simbolo, com_honorarios.imptot, " _
                + vbCr + " 'Compras' AS origen, 0 AS idori, com_honorarios.impsal, com_honorarios.idmon, com_honorarios.tipdoc, '' AS desccond, Format([fchdoc],'dd') AS dia, Format([fchdoc],'mm') AS mes, Format([fchdoc],'yy') AS ano  " _
                + vbCr + " FROM mae_prov RIGHT JOIN (mae_moneda RIGHT JOIN (mae_documento RIGHT JOIN com_honorarios ON mae_documento.id = com_honorarios.tipdoc) ON mae_moneda.id = com_honorarios.idmon) ON mae_prov.id = com_honorarios.idpro " _
                + vbCr + " WHERE com_honorarios.impsal<>0 " & nSQLIdPer & nSQLNotIn
        ...
        Case 10 '--reembolsables
            '--sentencia sql para mostrar documentos de proveedor
            If IdProveedor <> 0 Then nSQLIdPer = " AND com_reembolsables.idpro=" & IdProveedor & " "
            '--sentencia sql para no mostrar documentos seleccionados
            nSQLNotIn = GRID_GENERAR_SQL_ID(Fg3, 12, " AND com_reembolsables.id", " NOT IN", True)
            
            nSQL = "SELECT 0 as xsel ,com_reembolsables.id, mae_prov.nombre,mae_documento.abrev, mae_documento.codsun, com_reembolsables.fchdoc, " _
                + vbCr + " format(com_reembolsables.fchdoc,'mm/dd/yy') as fchdoc1, com_reembolsables.fchven, IIf([com_reembolsables]![numser] Is Null,'', " _
                + vbCr + " [com_reembolsables]![numser] & '-') & [com_reembolsables]![numdoc] AS numdoc, mae_moneda.simbolo, com_reembolsables.imptot, " _
                + vbCr + " 'Reembolsables' AS origen, 0 AS idori, com_reembolsables.impsal, com_reembolsables.idmon, com_reembolsables.tipdoc AS tipdoc, '' AS desccond, Format([fchdoc],'dd') AS dia, Format([fchdoc],'mm') AS mes, Format([fchdoc],'yy') AS ano  " _
                + vbCr + " FROM (mae_prov RIGHT JOIN (mae_moneda RIGHT JOIN com_reembolsables ON mae_moneda.id = com_reembolsables.idmon) ON mae_prov.id = com_reembolsables.idpro) LEFT JOIN mae_documento ON com_reembolsables.tipdoc = mae_documento.id " _
                + vbCr + " WHERE com_reembolsables.impsal<>0 " & nSQLIdPer & nSQLNotIn
        ...
        Case 11  '--Liquidacion de Gasto Debito
            '--sentencia sql para no mostrar documentos seleccionados
            nSQLNotIn = GRID_GENERAR_SQL_ID(Fg3, 12, " AND vta_gastodebito.id", " NOT IN", True)
            ...
            If IdProveedor = 0 Then
                nSQL = "SELECT 0 AS xSel,  vta_gastodebito.id, mae_cliente.nombre,  mae_documento.abrev,mae_documento.codsun, vta_gastodebito.fchemi AS fchdoc, vta_gastodebito.fchven, [vta_gastodebito]![numser]+'-'+[vta_gastodebito]![numdoc] AS numdoc, " _
                    ...
                    & " FROM ((vta_gastodebito LEFT JOIN mae_documento ON vta_gastodebito.tipdoc = mae_documento.id) LEFT JOIN mae_cliente ON vta_gastodebito.idcli = mae_cliente.id) " _
                    & " LEFT JOIN mae_moneda ON vta_gastodebito.idmon = mae_moneda.id " _
                    & " Where (((vta_gastodebito.impsal) <> 0)) " & nSQLNotIn _
                    & " ORDER BY [vta_gastodebito]![numser] +'-'+ [vta_gastodebito]![numdoc];"
            Else
                nSQL = "SELECT 0 AS xSel,  vta_gastodebito.id, mae_cliente.nombre,  mae_documento.abrev,mae_documento.codsun, vta_gastodebito.fchemi AS fchdoc, vta_gastodebito.fchven, [vta_gastodebito]![numser]+'-'+[vta_gastodebito]![numdoc] AS numdoc, " _
                    & " mae_moneda.simbolo, vta_gastodebito.imptot, 'LGD' AS origen, 2 AS idori, vta_gastodebito.impsal, vta_gastodebito.tipdoc, vta_gastodebito.idmon, '' AS desccond, Format([fchemi],'dd') AS dia, Format([fchemi],'mm') AS mes, Format([fchemi],'yy') AS ano  " _
                    & " FROM ((vta_gastodebito LEFT JOIN mae_documento ON vta_gastodebito.tipdoc = mae_documento.id) LEFT JOIN mae_cliente ON vta_gastodebito.idcli = mae_cliente.id) " _
                    & " LEFT JOIN mae_moneda ON vta_gastodebito.idmon = mae_moneda.id " _
                    & " WHERE (((vta_gastodebito.idcli)=" & IdProveedor & ") AND ((vta_gastodebito.impsal)<>0)) " & nSQLNotIn _
                    & " ORDER BY [vta_gastodebito]![numser] +'-'+ [vta_gastodebito]![numdoc]"
            End If
       ...
       Case 19  '--Planilla de letras
            ...
            '--sentencia sql para no mostrar documentos seleccionados
            nSQLNotIn = GRID_GENERAR_SQL_ID(Fg3, 12, " AND let_planilla.id", " NOT IN", True)
            
            If IdProveedor = 0 Then
                nSQL = "SELECT 0 AS xSel, let_planilla.id, Trim([mae_bancos].[descripcion]) & ' N° Cta. ' & Trim([mae_banconumcta].[numcue]) AS nombre, mae_documento.abrev, mae_documento.codsun, let_planilla.fchemi & '' AS fchdoc , Null AS fchvenc, let_planilla.numdoc, mae_moneda.simbolo, let_planilla.numlet, let_planilla.imptot, 'PLA LET' AS origen, 19 AS idori, let_planilla.impsal, let_planilla.tipdoc, let_planilla.idmon, '' AS desccond, Format([fchemi],'dd') AS dia, Format([fchemi],'mm') AS mes, Format([fchemi],'yy') AS ano  " _
                    + vbCr + " FROM mae_bancos RIGHT JOIN (mae_banconumcta RIGHT JOIN ((let_planilla LEFT JOIN mae_moneda ON let_planilla.idmon = mae_moneda.id) LEFT JOIN mae_documento ON let_planilla.tipdoc = mae_documento.id) ON mae_banconumcta.id = let_planilla.idbcocta) ON mae_bancos.id = mae_banconumcta.idban " _
                    + vbCr + " WHERE (((let_planilla.impsal)<>0)) " & nSQLNotIn
            Else
                nSQL = "SELECT 0 AS xSel, let_planilla.id, Trim([mae_bancos].[descripcion]) & ' N° Cta. ' & Trim([mae_banconumcta].[numcue]) AS nombre, mae_documento.abrev, mae_documento.codsun, let_planilla.fchemi & '' AS fchdoc , Null AS fchvenc, let_planilla.numdoc, mae_moneda.simbolo, let_planilla.numlet, let_planilla.imptot, 'PLA LET' AS origen, 19 AS idori, let_planilla.impsal, let_planilla.tipdoc, let_planilla.idmon, '' AS desccond, Format([fchemi],'dd') AS dia, Format([fchemi],'mm') AS mes, Format([fchemi],'yy') AS ano  " _
                    + vbCr + " FROM mae_bancos RIGHT JOIN (mae_banconumcta RIGHT JOIN ((let_planilla LEFT JOIN mae_moneda ON let_planilla.idmon = mae_moneda.id) LEFT JOIN mae_documento ON let_planilla.tipdoc = mae_documento.id) ON mae_banconumcta.id = let_planilla.idbcocta) ON mae_bancos.id = mae_banconumcta.idban " _
                    + vbCr + " WHERE (((let_planilla.impsal)<>0)) and let_planilla.idbcocta = " & IdProveedor & nSQLNotIn
            End If
            nTitulo = "Buscando Planilla de Letras"
        ...
        Case 20 '--orden de despacho
            '--sentencia sql para mostrar documentos de un cliente
            If IdProveedor <> 0 Then nSQLIdPer = " WHERE var_ordendespacho.idcli=" & IdProveedor & " "
            '--sentencia sql para no mostrar documentos seleccionados
            nSQLNotIn = nSQLIdPer & GRID_GENERAR_SQL_ID(Fg3, 12, " AND var_ordendespacho.id", " NOT IN", True)
            '--dar consitencia al filtro
            If InStr(nSQLNotIn, "WHERE") = 0 Then nSQLNotIn = Replace(nSQLNotIn, "AND", "WHERE")
        
                
            nSQL = "SELECT 0 as xsel, var_ordendespacho.id, mae_cliente.nombre, mae_documento.abrev, var_ordendespacho.fchdoc, Format([var_ordendespacho].[fchdoc],'mm/dd/yy') AS fchdoc1, " _
                + vbCr + " var_ordendespacho.fchven, var_ordendespacho.numdoc, '' AS simbolo, 0 AS imptot, 'Orden Despacho' AS origen, 0 AS idori, 0 AS impsal, " _
                + vbCr + " 0 AS idmon, var_ordendespacho.tipdoc, var_ordendespacho.imptot1, var_ordendespacho.imptot2, '' AS desccond , Format([fchdoc],'dd') AS dia, Format([fchdoc],'mm') AS mes, Format([fchdoc],'yy') AS ano  " _
                + vbCr + " FROM (var_ordendespacho INNER JOIN mae_cliente ON var_ordendespacho.idcli = mae_cliente.id) INNER JOIN mae_documento ON var_ordendespacho.tipdoc = mae_documento.id " _
                + vbCr + " " & nSQLNotIn & " ORDER BY var_ordendespacho.numdoc; "
