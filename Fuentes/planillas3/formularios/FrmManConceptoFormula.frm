...
Private Sub pCargarDatos()
    ...
    nSQL = "SELECT * FROM " _
        ...
        + vbCr + " WHERE (((pla_concepto.variable) Is Not Null)) and pla_concepto.activo = -1 and pla_conceptotipo.codsun Is Not Null " _
        + vbCr + " UNION " _
        + vbCr + " SELECT 'A99' AS catid, 'TIPO DE HORAS' AS catnombre, 'BB' AS tipid, ' ' AS tipnombre, 'F' & [id] as codigo ,id  , mae_tipohora.descripcion, mae_tipohora.variable, '' AS formula " _
        ...
        + vbCr + " UNION " _
        + vbCr + " SELECT 'C99' AS catid, 'SISTEMA' AS catnombre, 'DD' AS tipid, '' AS tipnombre, 'H' & [pla_concepto].[id] AS codigo, pla_concepto.id, pla_concepto.descripcion, pla_concepto.variable, pla_concepto.formula " _
        + vbCr + " FROM pla_conceptocat INNER JOIN (pla_conceptotipo INNER JOIN pla_concepto ON pla_conceptotipo.id = pla_concepto.idtipo) ON pla_conceptocat.id = pla_conceptotipo.idcat " _
        + vbCr + " WHERE (((pla_concepto.codsun)='' Or (pla_concepto.codsun) Is Null)) " _
        + vbCr + " ) AS vw " _
        + vbCr + " ORDER BY vw.catid, vw.tipnombre, vw.descripcion; "
    ...
    While Not RstConcepto.EOF
        ...
        If CodCatConcepto <> "A99" And CodCatConcepto <> "B99" And CodCatConcepto <> "C99" Then
