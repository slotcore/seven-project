...
Function Grabar() As Boolean
    ...
    xCon.Execute "DELETE FROM pla_conceptoemp WHERE (((pla_conceptoemp.anno)=" & AnoTra & ") AND ((pla_conceptoemp.idmes)= " & mMesActivo & " ) AND ((pla_conceptoemp.idemp)=" & NulosN(txt_cb(0).Text) & "));"
...
'        If RstTmp.RecordCount = 0 Then
            xCon.Execute "DELETE FROM pla_conceptoemp WHERE anno=" & AnoTra & " and idmes = " & mMesActivo & " and idemp= " & RstFrm.Fields("idemp") & " and idproc= " & RstFrm.Fields("idproc") & " and idcpto = " & RstFrm.Fields("idcpto")
'        End If
        Set RstTmp = Nothing
...
Sub Buscar()
    ...
    nSQL = "SELECT pla_conceptoemp.*, [pla_empleados].[apepat] & ' ' & [pla_empleados].[apemat] & ' ' & [pla_empleados].[nom] AS nombres, pla_proceso.abrev, pla_proceso.descripcion AS docnombre, pla_conceptocat.id AS catid, pla_conceptocat.descripcion AS catnombre, pla_concepto.descripcion " _
        ...
        + vbCr + " WHERE (((pla_conceptoemp.idmes) = " & mMesActivo & ") And ((pla_conceptoemp.anno) = " & AnoTra & ")) " _
...
Private Sub pCargarGrid()
    ...
    lblperiodo(1).Caption = Busca_Codigo(mMesActivo, "id", "descripcion", "con_meses", "N", xCon)
    Dim RstTmp As New ADODB.Recordset
...
Private Sub pCargarDatosDet()
    ...
    nSQL = "SELECT pla_conceptoemp.*, pla_empleados.apepat & ' ' & pla_empleados.apemat & ' ' & pla_empleados.nom AS nombres, pla_proceso.descripcion AS docnombre, pla_conceptotipo.idcat, pla_conceptocat.descripcion AS catnombre, pla_concepto.codsun, pla_concepto.descripcion AS cptonombre " _
        ...
        + vbCr + " WHERE (((pla_conceptoemp.idmes)=" & mMesActivo & ") AND ((pla_conceptoemp.anno)=" & AnoTra & ") AND ((pla_conceptoemp.idemp)=" & RstFrm.Fields("idemp") & " )) " _
    ...
    Select Case Index
        Case 0 '--personal
            ...
            nSQL = "SELECT pla_empleados.id, [pla_empleados].[apepat] & ' ' & [pla_empleados].[apemat] & ' ' & [pla_empleados].[nom] AS nombre, pla_empleados.id as cod,pla_empleados.numdoc, mae_dociden.abrev AS tipodoc, mae_sexo.abrev AS sexo, Format([pla_empleados].[fchnac],'dd/mm/yyyy') AS fchnac, pla_empleados.numtel, pla_empleados.email " _
                ...
                + vbCr + " and (pla_periodolaboral.fchfin Is Null) AND pla_empleados.id not in ( SELECT pla_conceptoemp.idemp From pla_conceptoemp WHERE (((pla_conceptoemp.anno)=" & AnoTra & " ) AND ((pla_conceptoemp.idmes)= " & mMesActivo & " and pla_conceptoemp.idemp <> " & NulosN(txt_cb(0).Text) & "))  );"
...
Private Sub pCargarPersonal()
    Dim xRs As New ADODB.Recordset
    pBuscarPersonal xRs, True, False
    If xRs.State = 1 Then
