...
'    dtcb_periodo.BoundText = 1
    txt_ano.Text = AnoTra
    dtcb_periodo.BoundText = mMesActivo
    '--------
    SeEjecuto = True
...
Sub Modificar()
    ...
    Agregando = False
    '---
    If opt(0).Value = True Then opt_Click 0
    If opt(1).Value = True Then opt_Click 1
...
Sub Eliminar()
    ...
    Dim nSQL As String
    nSQL = "SELECT pla_concepto.descripcion, pla_concepto.formula FROM pla_concepto WHERE (((pla_concepto.formula) like '% " & RstFrm("variable") & "%'));  "
    RST_Busq RstBus, nSQL, xCon
    
    If RstBus.RecordCount <> 0 Then
        MsgBox "No se puede eliminar el Registro, pues hay Conceptos Relacionados: " & vbCr & NulosC(RstBus.Fields("descripcion")), vbInformation + vbOKOnly + vbDefaultButton1, xTitulo
        Set RstBus = Nothing
        Exit Sub
    End If
    Set RstBus = Nothing
    ...
    If Rpta = vbYes Then
        xCon.Execute "DELETE FROM pla_conceptovariosdet WHERE idcptov=" & RstFrm.Fields("id") & " ;"
        xCon.Execute "DELETE FROM pla_conceptovarios WHERE id = " & RstFrm("id") & ""
...
Public Sub pCargarDatosDet()
    ...
    If NulosN(txt_ano.Text) = 0 Or NulosN(dtcb_periodo.BoundText) = 0 Then
        ...
        Exit Sub
    End If
    nSQL = "SELECT pla_conceptovariosdet.idref, IIf(pla_conceptovarios.entgen=1,mae_regimenpen.descripcion,IIf(pla_conceptovarios.entgen=2,mae_cargo.descripcion,IIf(pla_conceptovarios.entgen=3,con_meses.descripcion,'Varios'))) AS descripcion, pla_conceptovariosdet.imptot " _
        + vbCr + " FROM pla_conceptovarios INNER JOIN (((pla_conceptovariosdet LEFT JOIN mae_regimenpen ON pla_conceptovariosdet.idref = mae_regimenpen.id) LEFT JOIN mae_cargo ON pla_conceptovariosdet.idref = mae_cargo.id) LEFT JOIN con_meses ON pla_conceptovariosdet.idref = con_meses.id) ON pla_conceptovarios.id = pla_conceptovariosdet.idcptov " _
        ...
        + vbCr + " ORDER BY IIf(pla_conceptovarios.entgen=1,mae_regimenpen.descripcion,IIf(pla_conceptovarios.entgen=2,mae_cargo.descripcion,IIf(pla_conceptovarios.entgen=3,con_meses.descripcion,'Varios'))), pla_conceptovariosdet.imptot;"
...
Private Sub pConfigurarGrilla()
    With Fg1 '--
        ...
        .TextMatrix(0, 3) = "  Valor ":        .ColWidth(3) = 1300: .ColAlignment(3) = flexAlignRightCenter:  .Row = 0: .Col = 3: .CellAlignment = flexAlignRightCenter
        .ColFormat(3) = "#,###,###.#####"
...
Private Sub Fg1_CellButtonClick(ByVal Row As Long, ByVal Col As Long)
    ...
    Select Case NulosN(txt_cb(0).Text)
        ...
        Case 3 '--meses
            nSQL = "SELECT con_meses.id, con_meses.descripcion AS nombre  FROM con_meses WHERE (((con_meses.id)=" & NulosN(dtcb_periodo.BoundText) & ")); "
