...
Begin VB.Form FrmApruebaRequerimiento 
   Caption         =   "Compras - Aprobar Orden de Requerimiento"
   ClientHeight    =   6075
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   9780
   ...
   Begin XtremeSuiteControls.PushButton PushButton5 
      ...
      _ExtentY        =   1111
      _StockProps     =   79
      Caption         =   "Ver Orde de Requerimiento"
      UseVisualStyle  =   -1  'True
      Picture         =   "FrmApruebaRequerimiento.frx":0000
   End
   Begin XtremeSuiteControls.GroupBox GroupBox1 
      ...
      Begin XtremeSuiteControls.Label Label1 
         ...
         _ExtentY        =   529
         _StockProps     =   79
         Caption         =   "Orden de Requerimientos Emitidas"
         ForeColor       =   8388608
   ...
   Begin XtremeSuiteControls.PushButton PushButton1 
      ...
      _ExtentY        =   1111
      _StockProps     =   79
      Caption         =   "Aprobar Requerimiento"
      UseVisualStyle  =   -1  'True
      Picture         =   "FrmApruebaRequerimiento.frx":031A
   End
   Begin TrueOleDBGrid70.TDBGrid Dg1 
      ...
      Columns(0).Caption=   "Nº Requerimiento"
   ...
   Begin XtremeSuiteControls.PushButton PushButton2 
      ...
      _ExtentY        =   1111
      _StockProps     =   79
      Caption         =   "Rechazar Requerimiento"
      UseVisualStyle  =   -1  'True
      Picture         =   "FrmApruebaRequerimiento.frx":0634
   End
   Begin XtremeSuiteControls.PushButton PushButton3 
      ...
      Caption         =   "Salir"
      UseVisualStyle  =   -1  'True
      Picture         =   "FrmApruebaRequerimiento.frx":094E
   End
   Begin XtremeSuiteControls.PushButton PushButton4 
      ...
      UseVisualStyle  =   -1  'True
      Picture         =   "FrmApruebaRequerimiento.frx":0C68
   End
End
Attribute VB_Name = "FrmApruebaRequerimiento"
...
Private Sub Form_Activate()
    RST_Busq Rst, "SELECT com_ordenreq.id, [com_ordenreq]![numser] & '-' & [com_ordenreq]![numdoc] AS numdoc, com_ordenreq.fchemi, mae_area.descripcion, " _
        & " pla_empleados.nombre, com_ordenreq.idsit, mae_estados.descripcion AS descsit, com_ordenreq.idest, (SELECT Count(1) AS numitem " _
        & " From com_ordencotdet WHERE (((com_ordencotdet.idoc)=com_ordenreq.id))) AS numitem FROM (((com_ordenreq LEFT JOIN mae_area ON " _
        & " com_ordenreq.idarea = mae_area.id) LEFT JOIN com_usuario ON com_ordenreq.idsol = com_usuario.id) LEFT JOIN pla_empleados ON " _
        & " com_usuario.idper = pla_empleados.id) LEFT JOIN mae_estados ON com_ordenreq.idsit = mae_estados.id WHERE (((com_ordenreq.idest)=1))", xCon

    Dg1.DataSource = Rst
End Sub

Private Sub PushButton1_Click()
    xCon.Execute "UPDATE com_ordenreq SET com_ordenreq.idsit = 2 WHERE (((com_ordenreq.id)=" & Rst("id") & "))"
    Rst.Requery
    Dg1.Refresh
End Sub
...
'    If Rst("idsit") = 2 Then
'        MsgBox "No se puede desaprobar una orden de requerimientos aprobada", vbInformation + vbOKOnly + vbDefaultButton1, xTitulo
'        Exit Sub
'    End If
    xCon.Execute "UPDATE com_ordenreq SET com_ordenreq.idsit = 4 WHERE (((com_ordenreq.id)=" & Rst("id") & "))"
    Rst.Requery
    Dg1.Refresh
End Sub
...
Private Sub PushButton4_Click()
    xCon.Execute "UPDATE com_ordenreq SET com_ordenreq.idsit = 1 WHERE (((com_ordenreq.id)=" & Rst("id") & "))"
    Rst.Requery
    Dg1.Refresh
End Sub

Private Sub PushButton5_Click()
    FrmRequerimientos.DeDonde = 2
    FrmRequerimientos.xIdOR = Rst("id")
    FrmRequerimientos.Show vbModal
End Sub
